...
#    for dset in hmdb51 ucf101 diving48  mini_ssv2 uav ikea_furniture ;
    for dset in hmdb51 ucf101 diving48;
#    for dset in ikea_furniture;
#    for dset in hmdb51;
    do
...
#         for LR in 1.;
  ...
  #        python tools/submit.py --cfg configs/Downstream/${dset}_finetune.yaml --job_dir  expts/downstream/from_${backbone_dir}_${EPOCH}/${JOB_NAME}/  --num_shards 2 --num_gpus 4 --name ${JOB_NAME} MODEL.MODEL_NAME vit_base_patch16_224 TRAIN.CHECKPOINT_FILE_PATH /gpfs/u/home/DPLD/DPLDhwrg/scratch/TimeSformer/expts/${backbone_dir}/checkpoints/checkpoint_epoch_${EPOCH}.pyth
           JOB_NAME=ccc_${backbone_dir}_${dset}_lin_probe_lr${base_lr_str}
           python tools/submit.py --cfg configs/Downstream/${dset}_lin_probe.yaml \
           --job_dir  expts/downstream/hp_tune/from_ccc_${backbone_dir}_ep${EPOCH}/lin_probe/${JOB_NAME}/ \
